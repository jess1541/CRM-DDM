...
const ProceduresManager: React.FC<ProceduresManagerProps> = ({ procedures, doctors, onAddProcedure, onUpdateProcedure, onDeleteProcedure, user }) => {
  ...
  return (
    <div className="space-y-6 pb-10">
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-3xl border border-white/50 shadow-lg gap-4">
            <div>
            ...
            <div className="flex gap-3 items-center">
                <div className="bg-slate-100/50 p-1 rounded-xl flex shadow-inner border border-slate-200/50">
                   <button onClick={() => setViewMode('month')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${viewMode === 'month' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Mes</button>
                   <button onClick={() => setViewMode('week')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${viewMode === 'week' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Semana</button>
                   <button onClick={() => setViewMode('day')} className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${viewMode === 'day' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Día</button>
                </div>

                <button 
                    onClick={() => setIsListView(!isListView)} 
                    className={`p-2.5 rounded-xl transition-all ${isListView ? 'bg-rose-100 text-rose-600' : 'bg-white text-slate-500 hover:bg-slate-50 border border-slate-200'}`}
                    title={isListView ? "Ver Calendario" : "Ver Lista"}
                >
                    {isListView ? <CalendarIcon className="w-5 h-5" /> : <LayoutList className="w-5 h-5" />}
                </button>

                <button onClick={() => openModal()} className="bg-gradient-to-r from-red-600 to-rose-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg active:scale-95 transition-all flex items-center">
                                    ...
                                    <td className="p-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button 
                                                ...
                                                className="p-2 text-slate-300 hover:text-rose-600 transition-colors bg-white border border-slate-100 rounded-xl shadow-sm hover:shadow-md"
                                                title="Editar"
                                            >
                            ...
                            {calendarDays.map((day, idx) => {
                                ...
                                return (
                                    <div key={idx} 
                                         ...
                                         className={`${viewMode === 'day' ? 'h-full border rounded-2xl bg-slate-50/30 p-4' : 'min-h-[120px] p-2 border-b border-r border-slate-100'} hover:bg-red-50/20 transition-colors cursor-pointer relative group ${isToday && viewMode !== 'day' ? 'bg-red-50/30' : ''}`}
                                    >
                                        <div className="flex justify-between items-center mb-2">
                                            <span className={`text-sm font-bold w-7 h-7 flex items-center justify-center rounded-full ${isToday ? 'bg-rose-600 text-white shadow-md' : 'text-slate-700'}`}>
                                                {day.getDate()}
                                            </span>
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-3xl shadow-2xl w-[95%] md:w-full md:max-w-lg overflow-hidden animate-fadeIn scale-100 transform transition-all flex flex-col max-h-[90vh]">
                    ...
                    <div className={`p-6 border-b border-slate-100 flex justify-between items-center flex-shrink-0 bg-gradient-to-r ${formData.status === 'scheduled' ? 'from-slate-50 to-white' : 'from-green-50 to-white'}`}>
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-xl ${formData.status === 'scheduled' ? 'bg-rose-100 text-rose-600' : 'bg-green-100 text-green-600'}`}>
                    ...
                    <div className="p-6 md:p-8 space-y-6 overflow-y-auto flex-1">
                            ...
                            <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-200 shadow-inner">
                                ...
                                <button 
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs font-black uppercase transition-all shadow-sm ${
                                        ...
                                        : 'bg-gradient-to-r from-green-500 to-red-600 text-white shadow-green-500/30'
                                    }`}
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm text-slate-900 bg-white"
                                />
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <DatePicker
                                        ...
                                        dateFormat="h:mm aa"
                                        placeholderText="--:-- --"
                                        className="w-full pl-9 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm text-slate-900 bg-white"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-9 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm placeholder-slate-300 uppercase text-slate-900 bg-white"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <select 
                                    ...
                                    className="w-full pl-9 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm appearance-none cursor-pointer uppercase text-slate-900 bg-white"
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full pl-9 border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm placeholder-slate-300 text-slate-900 bg-white"
                                        placeholder="0.00"
                                    />
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="relative">
                                    <DollarSign className="absolute left-3 top-3 w-4 h-4 text-red-500" />
                                    <input 
                                        type="text"
                                        readOnly
                                        value={formData.commission ? new Intl.NumberFormat('es-MX', { minimumFractionDigits: 2 }).format(formData.commission) : '0.00'}
                                        className="w-full pl-9 bg-red-50 border border-red-100 text-red-800 rounded-xl p-3 text-sm font-bold outline-none cursor-default"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative group">
                               <Search className="absolute left-3 top-3.5 h-4 w-4 text-slate-400 group-focus-within:text-rose-500 transition-colors" />
                               <input 
                                   type="text" 
                                   placeholder="BUSCAR POR NOMBRE..." 
                                   value={searchTerm}
                                   onChange={e => { setSearchTerm(e.target.value.toUpperCase()); setFormData(prev => ({...prev, doctorId: ''})) }}
                                   className="w-full pl-10 border border-slate-200 rounded-xl p-3 text-sm font-bold uppercase focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm placeholder-slate-300 text-slate-900 bg-white"
                               />
                            </div>
                                ...
                                <div className="mt-2 max-h-40 overflow-y-auto border border-slate-100 rounded-xl bg-white shadow-lg w-full z-20">
                                    {filteredDoctors.map(doc => (
                                        <div 
                                            ...
                                            className="p-3 text-xs font-bold text-slate-600 hover:bg-rose-50 hover:text-rose-700 cursor-pointer uppercase border-b last:border-0 border-slate-50 transition-colors"
                                        >
                                            {doc.name}
                                        </div>
                                    ))}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input 
                                    ...
                                    className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold uppercase focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm placeholder-slate-300 text-slate-900 bg-white"
                            ...
                            <div>
                                ...
                                <select 
                                    ...
                                    className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-rose-500 outline-none transition-all shadow-sm uppercase text-slate-900 bg-white appearance-none cursor-pointer"
                                >
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                className="w-full border border-slate-200 rounded-xl p-3 text-sm font-medium uppercase focus:ring-2 focus:ring-rose-500 outline-none resize-none shadow-sm placeholder-slate-300 text-slate-900 bg-white"
                    ...
                    <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-between items-center flex-shrink-0">
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={handleSave} className="px-6 py-2.5 bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-700 hover:to-red-700 text-white font-bold rounded-xl shadow-lg shadow-rose-500/30 transition-all active:scale-95 text-sm flex items-center">
