  ...
  const renderEventChip = (evt: any, i: number, isSlot: boolean = false) => {
      ...
      const chipClasses = isSlot 
        ? `absolute left-0 right-0 mx-2 p-2 rounded shadow-sm cursor-pointer transition-colors z-10 border-l-4 ${
            isAppointment 
            ? 'bg-zinc-600 border-zinc-700 hover:bg-zinc-700 text-white' 
            : (isCompleted 
        ...
        : `px-2 py-1 rounded text-[8px] font-bold border shadow-sm flex items-center gap-1 transition-all hover:scale-[1.02] relative pr-1 cursor-grab active:cursor-grabbing w-full mb-1 ${
            ...
            : (isAppointment 
                ? 'bg-gradient-to-r from-zinc-500 to-rose-500 text-white border-transparent shadow-zinc-300' 
                : (isCompleted
                    ? 'bg-gradient-to-r from-red-500 to-red-500 text-white border-transparent shadow-red-300' 
                    : 'bg-gradient-to-r from-red-500 to-rose-600 text-white border-transparent shadow-red-200'))
        }`;
            ...
            {isSlot ? (
                <>
                    ...
                    <span className={`text-[10px] uppercase ${
                        isAppointment ? 'text-zinc-100' : (isCompleted ? 'text-red-100' : 'text-red-100')
    ...
    <div className="space-y-6 md:space-y-8 pb-10 relative animate-fadeIn">
       ...
       <div className="flex flex-col xl:flex-row justify-between items-center bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-200 shadow-xl gap-6">
           <div className="text-center xl:text-left w-full xl:w-auto">
               <h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight flex items-center justify-center xl:justify-start gap-3">
                   <CalendarClock className="w-8 h-8 md:w-10 md:h-10 text-red-600" />
                   CALENDARIO <span className="text-red-600">EJECUTIVO</span>
               </h1>
           ...
           <div className="flex flex-col md:flex-row gap-4 items-center w-full xl:w-auto">
               <div className="flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-auto border border-slate-200">
                   {(['month', 'week', 'day'] as ViewMode[]).map(mode => (
                       <button
                           ...
                           className={`flex-1 md:flex-none px-6 md:px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${viewMode === mode ? 'bg-white text-red-600 shadow-md' : 'text-slate-500 hover:text-slate-800'}`}
                       >
               ...
               <div className="flex gap-3 w-full md:w-auto overflow-x-auto no-scrollbar pb-1">
                   <button 
                       ...
                       className="flex-1 md:flex-none bg-red-600 hover:bg-red-500 text-white px-6 md:px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-red-500/20 transition-all active:scale-95 flex items-center justify-center whitespace-nowrap"
                   >
                   ...
                   <button 
                       ...
                       className="flex-1 md:flex-none bg-zinc-600 hover:bg-zinc-500 text-white px-6 md:px-8 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-zinc-500/20 transition-all active:scale-95 flex items-center justify-center whitespace-nowrap"
                   >
       ...
       <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-200 gap-6">
           <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
               <button onClick={prevPeriod} className="p-3 hover:bg-slate-100 rounded-2xl transition-colors text-slate-400 hover:text-red-600"><ChevronLeft className="w-6 h-6" /></button>
               ...
               <button onClick={nextPeriod} className="p-3 hover:bg-slate-100 rounded-2xl transition-colors text-slate-400 hover:text-red-600"><ChevronRight className="w-6 h-6" /></button>
           </div>
           ...
           <div className="flex items-center gap-3 w-full md:w-auto">
               <div className="relative flex-1 md:flex-none">
                   <select 
                       ...
                       className="w-full md:w-64 appearance-none bg-slate-50 border border-slate-200 text-slate-700 text-xs font-black uppercase tracking-wide rounded-2xl py-3 pl-4 pr-10 focus:outline-none focus:ring-2 focus:ring-red-500/20"
               ...
               <button onClick={() => setCurrentDate(new Date())} className="px-4 py-3 bg-red-50 text-red-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-colors whitespace-nowrap">
                   Hoy
               </button>
           </div>
       </div>
                       ...
                       {calendarDays.map((day, idx) => {
                                           ...
                                           {visitTimeSlots.map(time => (
                                               <div key={time} className="h-24 text-xs font-bold text-slate-500 flex items-start justify-between group relative">
                                                   ...
                                                   <button 
                                                        ...
                                                        className="absolute right-0 top-0 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white rounded-full p-1.5 transition-all transform active:scale-95 z-20 md:opacity-0 md:group-hover:opacity-100 border border-red-100"
                           ...
                           return (
                               <div key={idx} 
                                   ...
                                   className={`min-h-[140px] bg-white p-3 border-b border-r border-slate-100 hover:bg-slate-50 transition-all cursor-pointer relative group flex flex-col gap-2 ${isToday ? 'bg-red-50 ring-1 ring-inset ring-red-100' : ''}`}
                               >
                                   <div className="flex justify-between items-start">
                                        <span className={`text-xs font-black w-7 h-7 flex items-center justify-center rounded-full transition-all ${isToday ? 'bg-red-600 text-white shadow-md' : 'text-slate-500 group-hover:text-slate-800'}`}>
                                            {day.getDate()}
                                        </span>
                                        <button className="opacity-0 group-hover:opacity-100 bg-slate-100 p-1.5 rounded-xl text-slate-500 hover:text-red-600 transition-all border border-slate-200">
               ...
               <div className="bg-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-2xl w-[95%] md:w-full md:max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                   <div className="p-8 border-b border-white/10 flex justify-between items-center bg-white/5">
                       <h3 className="text-xl font-black text-white flex items-center gap-3">
                           {isAppointmentMode ? <Clock className="w-6 h-6 text-zinc-500" /> : <Calendar className="w-6 h-6 text-red-500" />}
                   ...
                   <div className="p-8 space-y-6 overflow-y-auto">
                        {isAppointmentMode && (
                           <div className="bg-zinc-500/10 p-4 rounded-2xl border border-zinc-500/20 flex items-center text-zinc-400 text-xs font-bold">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input 
                                    ...
                                    className="w-full pl-12 pr-4 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-white placeholder:text-slate-600 focus:ring-2 focus:ring-red-500 outline-none transition-all"
                                />
                            </div>
                            {searchDoctorTerm && (
                                <div className="mt-2 max-h-48 overflow-y-auto border border-white/10 rounded-2xl bg-slate-900/90 backdrop-blur-xl shadow-2xl">
                                    {filteredDoctorsForModal.map(doc => (
                                        <div 
                                            ...
                                            className={`p-4 text-xs font-bold border-b border-white/5 last:border-0 cursor-pointer transition-colors ${selectedDoctorId === doc.id ? 'bg-red-600/20 text-red-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                                        >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <div className="space-y-2">
                               ...
                               <DatePicker
                                   ...
                                   locale="es"
                                   className={`w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all ${!!editingAppointment ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10'}`}
                               />
                           </div>
                           ...
                           <div className="space-y-2">
                               ...
                               <select 
                                   ...
                                   disabled={!!editingAppointment}
                                   className={`w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all appearance-none ${!!editingAppointment ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10'}`}
                               >
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea 
                                ...
                                className={`w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm uppercase font-bold text-white placeholder:text-slate-600 focus:ring-2 focus:ring-red-500 outline-none resize-none transition-all ${isAppointmentMode || !!editingAppointment ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/10'}`}
                   ...
                   <div className="p-8 border-t border-white/10 flex justify-end gap-4 bg-white/5">
                       ...
                       <button onClick={savePlan} className="px-8 py-3 bg-red-600 hover:bg-red-500 text-white rounded-2xl text-sm font-black uppercase tracking-widest shadow-xl shadow-red-500/20 transition-all active:scale-95">Guardar Plan</button>
                   </div>
               </div>
           </div>
       )}
           ...
           <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
               <div className="bg-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/10 shadow-2xl w-[95%] md:w-full md:max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
                   <div className="p-8 border-b border-white/10 flex justify-between items-center bg-white/5">
                       <div>
                           <div className="flex space-x-6 mb-3">
                                <button onClick={() => setIsEditingPlan(false)} className={`text-sm font-black uppercase border-b-2 pb-2 transition-all tracking-widest ${!isEditingPlan ? 'text-red-500 border-red-500' : 'text-slate-500 border-transparent hover:text-slate-300'}`}>
                                     Reportar
                                </button>
                                <button onClick={() => setIsEditingPlan(true)} className={`text-sm font-black uppercase border-b-2 pb-2 transition-all tracking-widest ${isEditingPlan ? 'text-red-500 border-red-500' : 'text-slate-500 border-transparent hover:text-slate-300'}`}>
                                     Reprogramar
                                </button>
                           </div>
                   ...
                   {selectedVisitToReport && (() => {
                       const doc = doctors.find(d => d.id === selectedVisitToReport.docId);
                       return doc ? (
                           <div className="px-8 py-6 bg-red-500/5 border-b border-white/10 flex items-center gap-4">
                               <div className="p-3 bg-white/5 rounded-2xl border border-white/10 text-red-400 shadow-xl">
                                   <UserIcon className="w-6 h-6" />
                               </div>
                               <div>
                                   <h4 className="text-base font-black text-white uppercase leading-tight tracking-wide">{doc.name}</h4>
                                   <p className="text-xs font-bold text-red-400 uppercase flex items-center gap-2 mt-1">
                   ...
                   <div className="p-8 space-y-8 overflow-y-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Fecha</label>
                                <input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all" />
                            </div>
                            <div className="space-y-2">
                                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Hora</label>
                                <select value={reportTime} onChange={(e) => setReportTime(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all appearance-none">
                        ...
                        {isEditingPlan ? (
                            <div className="space-y-2">
                                ...
                                <textarea 
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none uppercase resize-none transition-all hover:bg-white/10"
                                />
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <select value={reportOutcome} onChange={(e) => setReportOutcome(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all appearance-none">
                                ...
                                <div className="border-t border-white/10 pt-8 bg-red-500/5 p-6 rounded-[2rem] border border-white/5">
                                    <div className="mb-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                        <div className="space-y-2">
                                            <label className="block text-[10px] font-black text-red-400 uppercase tracking-widest ml-1">Fecha</label>
                                            <DatePicker 
                                                 ...
                                                 className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-xs font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all hover:bg-white/10"
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="block text-[10px] font-black text-red-400 uppercase tracking-widest ml-1">Hora</label>
                                            <select 
                                                 value={nextVisitTime} 
                                                 onChange={(e) => setNextVisitTime(e.target.value)}
                                                 className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-xs font-bold text-white focus:ring-2 focus:ring-red-500 outline-none transition-all appearance-none"
                                            >
                                                {visitTimeSlots.map(t => <option key={t} value={t} className="bg-slate-900 text-white">{t}</option>)}
                                            </select>
                                        </div>
                   ...
                   <div className="p-8 border-t border-white/10 flex justify-end gap-4 bg-white/5">
                       ...
                       {isEditingPlan ? (
                           <button onClick={savePlanChanges} className="px-8 py-3 bg-red-600 hover:bg-red-500 text-white rounded-2xl text-sm font-black uppercase tracking-widest shadow-xl shadow-red-500/20 transition-all active:scale-95">Guardar Cambios</button>
                       ) : (
