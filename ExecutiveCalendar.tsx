  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-200 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-red-500/5 to-rose-500/5 rounded-full blur-3xl -z-10 opacity-60"></div>
        
        <div className="relative z-10">
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                <div className="p-3 bg-red-50 text-red-600 rounded-2xl border border-red-100 backdrop-blur-md">
                    <Database className="w-6 h-6" />
                </div>
                Directorio <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-rose-600">Central</span>
            </h1>
        ...
        <div className="grid grid-cols-2 md:flex md:flex-wrap gap-2 md:gap-3 relative z-10">
            ...
            <button 
                ...
                className="flex items-center justify-center px-4 py-3 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm font-black text-[10px] uppercase tracking-widest active:scale-95 border border-rose-100"
            >
            ...
            <button 
                ...
                className="flex items-center justify-center px-4 py-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm font-black text-[10px] uppercase tracking-widest active:scale-95 border border-red-100"
            >
            ...
            <button 
                ...
                className="col-span-2 md:col-span-1 flex items-center justify-center px-6 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl hover:from-red-500 hover:to-red-600 transition-all shadow-lg shadow-red-500/20 font-black text-[10px] uppercase tracking-widest active:scale-95 border border-white/10"
            >
      ...
      <div className="bg-white p-4 md:p-6 rounded-[2.5rem] shadow-lg border border-slate-200 space-y-4 md:space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2 p-1.5 bg-slate-50 rounded-2xl w-full md:w-fit overflow-x-auto no-scrollbar border border-slate-100">
                {(['MEDICO', 'ADMINISTRATIVO', 'HOSPITAL'] as TabType[]).map(tab => (
                    <button
                        ...
                        className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-red-600 text-white shadow-lg shadow-red-500/20 ring-1 ring-white/20' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}
                    >
            ...
            {user.role === 'admin' && filteredItems.length > 0 && (
                ...
                >
                    <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${selectedIds.length === filteredItems.length ? 'bg-red-600 border-red-500' : 'bg-white border-slate-200'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 relative group">
                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                    <Search className="h-5 w-5 text-slate-300 group-focus-within:text-red-600 transition-colors" />
                </div>
                <input
                    type="text"
                    className="block w-full pl-14 pr-4 py-4 border border-slate-200 rounded-2xl bg-slate-50 text-slate-900 font-bold placeholder-slate-300 focus:outline-none focus:border-red-500/50 focus:ring-4 focus:ring-red-500/10 transition-all uppercase text-sm"
            ...
            <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                    <Filter className="h-5 w-5 text-slate-300 group-focus-within:text-red-600 transition-colors" />
                </div>
                <select
                    className="block w-full pl-14 pr-10 py-4 border border-slate-200 rounded-2xl bg-slate-50 text-slate-900 font-bold focus:outline-none focus:border-red-500/50 focus:ring-4 focus:ring-red-500/10 transition-all uppercase text-sm appearance-none cursor-pointer"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {itemsToShow.map((item) => (
              <div
                ...
                className={`bg-white rounded-[2.5rem] shadow-lg border p-8 hover:shadow-red-500/10 hover:-translate-y-2 transition-all cursor-pointer group flex flex-col h-full relative overflow-hidden ${selectedIds.includes(item.id) ? 'border-red-500 ring-2 ring-red-500/20' : 'border-slate-100'}`}
              >
                <div className={`absolute top-0 right-0 w-32 h-32 -mr-16 -mt-16 rounded-full opacity-5 group-hover:opacity-10 transition-opacity ${activeTab === 'MEDICO' ? 'bg-red-600' : 'bg-red-600'}`}></div>
                    ...
                    <div className="flex items-center gap-4">
                        {user.role === 'admin' && (
                            <div 
                                ...
                                className={`w-6 h-6 rounded-lg border flex items-center justify-center transition-all z-20 ${selectedIds.includes(item.id) ? 'bg-red-600 border-red-500 shadow-lg shadow-red-500/30' : 'bg-slate-50 border-slate-200 hover:border-red-400'}`}
                            >
                                {selectedIds.includes(item.id) && <Plus className="w-4 h-4 text-white rotate-45" />}
                            </div>
                        )}
                        <div className={`p-4 rounded-2xl text-white shadow-lg ${activeTab === 'MEDICO' ? 'bg-gradient-to-br from-red-600 to-red-700' : activeTab === 'ADMINISTRATIVO' ? 'bg-gradient-to-br from-rose-600 to-red-700' : 'bg-gradient-to-br from-red-600 to-red-700'}`}>
                ...
                <h3 className="text-base font-black text-slate-900 uppercase leading-tight group-hover:text-red-600 transition-colors line-clamp-2 mb-2">{item.name}</h3>
                ...
                <div className="mt-6 flex justify-end">
                    <div className="w-10 h-10 rounded-full bg-red-600 text-white flex items-center justify-center transform translate-x-4 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all shadow-xl shadow-red-500/20">
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg overflow-hidden animate-fadeIn border border-slate-200">
                  ...
                  <form onSubmit={handleAddSubmit} className="p-10 space-y-6">
                      <div>
                          ...
                          <input type="text" required className="w-full border border-slate-200 bg-slate-50 rounded-2xl p-4.5 font-black uppercase text-sm text-slate-900 focus:ring-4 focus:ring-red-500/10 outline-none transition-all shadow-inner" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                      </div>
                      <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Especialidad</label>
                          <input type="text" className="w-full border border-slate-200 bg-slate-50 rounded-2xl p-4.5 font-black uppercase text-sm text-slate-900 focus:ring-4 focus:ring-red-500/10 outline-none transition-all shadow-inner" value={formData.specialty} onChange={e => setFormData({...formData, specialty: e.target.value})} />
                      </div>
                      <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Dirección</label>
                          <textarea required rows={3} className="w-full border border-slate-200 bg-slate-50 rounded-2xl p-4.5 font-black uppercase text-sm text-slate-900 focus:ring-4 focus:ring-red-500/10 outline-none transition-all shadow-inner resize-none" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} />
                      </div>
                      {user.role === 'admin' && (
                          <div>
                              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1">Ejecutivo Asignado</label>
                              <select 
                                  className="w-full border border-slate-200 bg-slate-50 rounded-2xl p-4.5 font-black uppercase text-sm text-slate-900 focus:ring-4 focus:ring-red-500/10 outline-none transition-all shadow-inner appearance-none cursor-pointer"
                      ...
                      <div className="flex justify-end gap-4 mt-8">
                          ...
                          <button type="submit" className="px-10 py-4 bg-red-600 text-white rounded-[1.5rem] font-black uppercase text-xs tracking-widest shadow-xl shadow-red-500/30 active:scale-95 transition-all border border-white/10">Guardar</button>
                      </div>
                  </form>
              </div>
          </div>
