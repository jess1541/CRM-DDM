...
const DoctorProfile: React.FC<DoctorProfileProps> = ({ doctors, onUpdate, onDeleteVisit, user }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-10">
      <button type="button" onClick={() => fromParam === 'calendar' ? navigate('/calendar') : navigate('/doctors')} className="flex items-center text-sm font-bold text-slate-500 hover:text-red-600 transition-colors bg-white backdrop-blur px-4 py-2 rounded-xl shadow-sm border border-slate-200">
        <ArrowLeft className="h-4 w-4 mr-2" /> {fromParam === 'calendar' ? 'Volver al Calendario' : 'Volver al Directorio'}
      </button>

      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-red-500/5 border border-slate-200 overflow-hidden relative">
        <div className="h-4 bg-gradient-to-r from-red-600 via-rose-500 to-purple-600"></div>
        <div className="p-6 md:p-8 md:flex md:items-center md:justify-between">
          <div className="flex-1 min-w-0">
             <div className="flex items-center flex-wrap gap-3">
                ...
                {isHospital && <span className="bg-red-50 text-red-700 px-2 py-1 rounded text-[10px] md:text-xs font-bold uppercase border border-red-100">Hospital</span>}
             </div>
            <p className="mt-4 text-xs md:text-sm text-slate-500 flex flex-wrap items-center font-medium gap-2">
                {(isMedico || isAdminCat) && (<span className="font-bold text-red-700 bg-red-50 px-3 py-1.5 rounded-lg uppercase tracking-wide shadow-sm border border-red-100">{doctor.specialty || 'GENERAL'}</span>)}
                {doctor.hospital && (<span className="font-bold text-red-700 bg-red-50 px-3 py-1.5 rounded-lg uppercase tracking-wide shadow-sm border border-red-100 flex items-center"><Building2 className="w-3.5 h-3.5 mr-1.5" />{doctor.hospital}</span>)}
          ...
          {activeTab === 'profile' && (
            <div className="space-y-10 animate-fadeIn">
                <div className="flex justify-between items-center pb-4 border-b border-slate-100 sticky top-0 bg-white z-20 backdrop-blur-sm">
                     <h3 className="text-xl font-bold text-slate-800 flex items-center"><FileText className="w-5 h-5 mr-2 text-red-500"/>Información Completa</h3>
                     {!isEditing ? (
                         doctor.status !== 'archived' && (
                             <button type="button" onClick={() => setIsEditing(true)} className="text-sm font-bold bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-red-600 py-2.5 px-5 rounded-xl transition-all shadow-sm flex items-center"><Edit3 className="w-4 h-4 mr-2"/>Editar Ficha</button>
                         )
                     ) : (
                         <div className="flex space-x-3">
                            ...
                            <button type="button" onClick={saveProfile} className="flex items-center text-sm font-bold bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white py-2.5 px-5 rounded-xl shadow-lg shadow-red-500/30 transition-all active:scale-95"><Save className="w-4 h-4 mr-2" />Guardar Cambios</button>
                         </div>
                     )}
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                             <div className="lg:col-span-1">
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2 flex items-center">
                                    <UserPlus className="w-3 h-3 mr-1 text-red-500" /> Ejecutivo Asignado
                                </label>
                                {isEditing && user.role === 'admin' ? (
                                    <select name="executive" value={formData.executive || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-bold uppercase shadow-sm">
                                    ...
                                    <p className="text-black font-black text-base uppercase bg-slate-50 p-3 rounded-xl border border-slate-100 flex items-center justify-between">
                                        {doctor.executive}
                                        {user.role === 'admin' && !isEditing && <span className="text-[8px] font-bold text-red-400 bg-white px-2 py-0.5 rounded border border-red-100">EDITABLE POR ADMIN</span>}
                                    </p>
                                )}
                            </div>
                            <div className="lg:col-span-2">
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Nombre</label>
                                {isEditing ? (<input spellCheck={true} lang="es" type="text" name="name" value={formData.name || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" />) : (<p className="text-black font-bold text-base uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.name}</p>)}
                            </div>
                            <div className="lg:col-span-3">
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Dirección</label>
                                {isEditing ? (<input spellCheck={true} lang="es" type="text" name="address" value={formData.address || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.address}</p>)}
                            </div>
                            <div>
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Fecha de Nacimiento</label>
                                {isEditing ? (<DatePicker selected={parseDateString(formData.birthDate || '')} onChange={(date) => handleDateChange(date, 'birthDate')} dateFormat="dd/MM/yyyy" locale="es" className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium shadow-sm" placeholderText="DD/MM/AAAA" showYearDropdown scrollableYearDropdown yearDropdownItemNumber={100} />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent flex items-center"><Calendar className="w-3 h-3 mr-2 text-slate-400"/>{doctor.birthDate ? parseDateString(doctor.birthDate)?.toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }) : 'NO REGISTRADA'}</p>)}
                            </div>
                            <div>
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Cédula Profesional</label>
                                {isEditing ? (<input type="text" name="cedula" value={formData.cedula || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" placeholder="#######" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent flex items-center"><CreditCard className="w-3 h-3 mr-2 text-slate-400"/>{doctor.cedula || 'NO REGISTRADA'}</p>)}
                            </div>
                            <div>
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Teléfono</label>
                                {isEditing ? (<input type="text" name="phone" value={formData.phone || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium shadow-sm" placeholder="### ### ####" />) : (<p className="text-black font-medium bg-slate-50 p-3 rounded-xl border border-transparent flex items-center"><Phone className="w-3 h-3 mr-2 text-slate-400"/> {doctor.phone || 'NO REGISTRADO'}</p>)}
                            </div>
                            <div>
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Correo Electrónico</label>
                                {isEditing ? (<input type="email" name="email" value={formData.email || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium shadow-sm" />) : (<p className="text-black font-medium bg-slate-50 p-3 rounded-xl border border-transparent flex items-center lowercase"><Mail className="w-3 h-3 mr-2 text-slate-400"/> {doctor.email || 'NO REGISTRADO'}</p>)}
                            </div>
                            <div>
                                <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Hospital / Ubicación</label>
                                {isEditing ? (<input spellCheck={true} lang="es" type="text" name="hospital" value={formData.hospital || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.hospital || 'NO REGISTRADO'}</p>)}
                            </div>
                            {!isHospital && (<><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Piso</label>{isEditing ? (<input type="text" name="floor" value={formData.floor || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" placeholder="EJ: 3" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.floor || 'N/A'}</p>)}</div><div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Oficina</label>{isEditing ? (<input type="text" name="officeNumber" value={formData.officeNumber || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" placeholder="EJ: 305" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.officeNumber || 'N/A'}</p>)}</div></div></>)}
                            {isMedico && isEditing && (
                                <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-200 mt-2"><span className="text-xs font-extrabold text-slate-500 uppercase">¿Es Médico de Aseguradora?</span><label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" className="sr-only peer" checked={formData.isInsuranceDoctor || false} onChange={(e) => setFormData({...formData, isInsuranceDoctor: e.target.checked})} /><div className="w-11 h-6 bg-slate-300 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-red-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-600"></div></label></div>
                            )}
                        </div>
                    </div>
                    {isMedico && (
                        <div className="lg:col-span-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Especialidad</label>{isEditing ? (<input spellCheck={true} lang="es" type="text" name="specialty" value={formData.specialty || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.specialty || 'GENERAL'}</p>)}</div>
                                <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Subespecialidad</label>{isEditing ? (<input spellCheck={true} lang="es" type="text" name="subSpecialty" value={formData.subSpecialty || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm" placeholder="EJ: ONCOLOGÍA" />) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.subSpecialty || 'N/A'}</p>)}</div>
                                <div>
                                    <label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Categoría (Clasificación)</label>
                                    {isEditing ? (
                                        <select name="classification" value={formData.classification || 'C'} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium uppercase shadow-sm">
                    ...
                    {isMedico && (
                        <div className="lg:col-span-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                                <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Estilo Social</label>{isEditing ? (<select name="socialStyle" value={formData.socialStyle || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium shadow-sm"><option value="">SELECCIONAR</option><option value="ANALÍTICO">ANALÍTICO</option><option value="EMPRENDEDOR">EMPRENDEDOR</option><option value="AFABLE">AFABLE</option><option value="EXPRESIVO">EXPRESIVO</option></select>) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.socialStyle || 'NO DEFINIDO'}</p>)}</div>
                                <div><label className="block text-xs font-extrabold text-slate-500 uppercase mb-2">Segmento Actitudinal</label>{isEditing ? (<select name="attitudinalSegment" value={formData.attitudinalSegment || ''} onChange={handleInputChange} className="block w-full border border-slate-200 rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 bg-white font-medium shadow-sm"><option value="">SELECCIONAR</option><option value="RELACIÓN">RELACIÓN</option><option value="PACIENTE">PACIENTE</option><option value="INNOVACIÓN">INNOVACIÓN</option><option value="EXPERIENCIA">EXPERIENCIA</option></select>) : (<p className="text-black font-medium uppercase bg-slate-50 p-3 rounded-xl border border-transparent">{doctor.attitudinalSegment || 'NO DEFINIDO'}</p>)}</div>
                            </div>
                        </div>
                    )}
                    <div className="lg:col-span-3">
          ...
          {activeTab === 'visits' && (
            <div className="space-y-8 animate-fadeIn">
               <div className="bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden">
                   <div className="flex border-b border-slate-200 bg-white p-2 gap-2"><button onClick={() => setVisitType('report')} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${visitType === 'report' ? 'bg-gradient-to-r from-red-50 to-rose-50 text-red-700 shadow-inner' : 'text-slate-500 hover:bg-slate-50'}`}>Reportar Visita</button><button onClick={() => setVisitType('plan')} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${visitType === 'plan' ? 'bg-gradient-to-r from-red-50 to-rose-50 text-red-700 shadow-inner' : 'text-slate-500 hover:bg-slate-50'}`}>Planear Siguiente</button></div>
                    <div className="p-8">
                        <form onSubmit={handleAddInteraction} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <DatePicker selected={parseDateString(newVisit.date || '')} onChange={(date) => setNewVisit({...newVisit, date: formatDateToString(date)})} dateFormat="dd/MM/yyyy" locale="es" showMonthDropdown showYearDropdown dropdownMode="select" className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 font-medium shadow-sm" required />
                                </div>
                                {visitType === 'plan' && (
                                    <div>
                                        <label className="block text-xs font-extrabold text-slate-400 uppercase tracking-widest mb-2">Hora</label>
                                        <select value={newVisit.time} onChange={(e) => setNewVisit({...newVisit, time: e.target.value})} className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 font-medium shadow-sm">
                                ...
                                {visitType === 'report' && (
                                    <div>
                                        ...
                                        <select value={newVisit.outcome || 'SEGUIMIENTO'} onChange={(e) => setNewVisit({...newVisit, outcome: e.target.value as any})} className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 font-medium shadow-sm">
                            ...
                            <div>
                                ...
                                <textarea spellCheck={true} lang="es" value={newVisit.objective || ''} onChange={(e) => setNewVisit({...newVisit, objective: e.target.value.toUpperCase()})} placeholder="DESCRIBA EL PROPÓSITO DE LA VISITA..." rows={2} className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 resize-none uppercase shadow-sm" />
                            </div>
                            {visitType === 'report' && (
                                <>
                                    <div>
                                        <label className="block text-xs font-extrabold text-slate-400 uppercase tracking-widest mb-2">Reporte / Resultado</label>
                                        <textarea spellCheck={true} lang="es" value={newVisit.note || ''} onChange={(e) => setNewVisit({...newVisit, note: e.target.value.toUpperCase()})} placeholder="DETALLES RELEVANTES DE LA INTERACCIÓN..." rows={3} className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 resize-none uppercase shadow-sm" />
                                    </div>
                                    <div>
                                        ...
                                        <textarea spellCheck={true} lang="es" value={newVisit.followUp || ''} onChange={(e) => setNewVisit({...newVisit, followUp: e.target.value.toUpperCase()})} placeholder="COMPROMISOS O ACCIONES A SEGUIR..." rows={2} className="block w-full border border-slate-200 bg-white rounded-xl p-3 text-sm text-black focus:ring-2 focus:ring-red-500 resize-none uppercase shadow-sm" />
                                    </div>
                            ...
                            <div className="flex justify-end pt-2">
                                <button type="submit" className={`px-8 py-3 rounded-xl text-white font-bold shadow-lg transition-all active:scale-95 ${visitType === 'plan' ? 'bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 shadow-red-500/30' : 'bg-gradient-to-r from-green-600 to-red-600 hover:from-green-700 hover:to-red-700 shadow-green-500/30'}`}>
               ...
               <div className="mt-10">
                   <h4 className="text-xl font-black text-slate-800 mb-6 pl-3 border-l-4 border-red-500">Historial de Visitas</h4>
                   {doctor.visits.filter(v => v.status === 'completed' || !v.status).length > 0 ? (
                       <ul className="space-y-6 relative before:absolute before:inset-0 before:ml-8 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-slate-200 before:to-transparent">
                           {doctor.visits.filter(v => v.status === 'completed' || !v.status).map((visit) => (
                               <li key={visit.id} className="relative pl-16 group"><div className="absolute left-4 top-5 w-8 h-8 rounded-full bg-white border-4 border-red-500 shadow-md flex items-center justify-center z-10"><div className="w-2.5 h-2.5 bg-red-500 rounded-full"></div></div>
                                   <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 relative pr-12"><button type="button" onClick={(e) => { e.stopPropagation(); confirmDelete(visit.id); }} className="absolute top-4 right-4 p-2 bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-500 rounded-xl transition-all shadow-sm z-10 cursor-pointer" title="Eliminar este reporte"><Trash2 className="h-4 w-4" /></button>
                                       <div className="flex flex-col md:flex-row justify-between items-start mb-4 gap-2"><div className="flex items-center"><div><span className="block text-lg font-bold text-slate-800">{visit.date}</span>{visit.time && <span className="text-sm text-red-600 font-bold bg-red-50 px-2 py-0.5 rounded-lg mr-2">{visit.time}</span>}<span className="text-xs font-bold text-slate-400 uppercase tracking-wide">Visita Completada</span></div></div><span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wide shadow-sm ${visit.outcome === 'INTERESADO' ? 'bg-green-100 text-green-700' : visit.outcome === 'COTIZACIÓN' ? 'bg-red-100 text-red-700' : visit.outcome === 'CITA' ? 'bg-zinc-100 text-zinc-700' : 'bg-slate-100 text-slate-700'}`}>{visit.outcome}</span></div>
                                       <div className="space-y-3">
                                           ...
                                           {visit.followUp && (<div className="text-sm pt-1 flex items-start"><span className="font-bold text-red-600 mr-2 min-w-[80px]">Seguimiento:</span><span className="text-slate-600 font-medium uppercase">{visit.followUp}</span></div>)}
                                           {visit.checkIn && (
