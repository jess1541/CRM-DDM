...
import { LayoutDashboard, Users, CalendarDays, User, LogOut, Shield, ChevronLeft, ChevronRight, Activity, RefreshCw, Settings } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ user, onLogout, isMobileOpen, closeMobileMenu, isCollapsed, toggleCollapse, isOnline = true, isSyncing = false }) => {
  ...
  const executives = [
      { name: 'ESPECIALISTA1', color: 'from-red-400 to-red-600', initials: 'E1' },
      { name: 'ESPECIALISTA2', color: 'from-zinc-400 to-rose-600', initials: 'E2' },
      { name: 'ESPECIALISTA3', color: 'from-red-400 to-red-600', initials: 'E3' },
      { name: 'ESPECIALISTA4', color: 'from-purple-400 to-rose-600', initials: 'E4' }
  ];
        ...
        <div className={`flex flex-col bg-white text-slate-700 shadow-2xl overflow-hidden transition-all duration-500 ease-in-out z-50 fixed inset-y-0 left-0 h-full border-r border-slate-200 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isCollapsed ? 'w-20' : 'w-72'}`}>
            ...
            <div className="flex flex-col items-center justify-center h-24 border-b border-slate-100 relative z-10 mt-safe-top md:mt-0">
                <div className="text-center group cursor-default flex items-center justify-center w-full h-full relative">
                    {isSyncing && (
                        <div className="absolute top-4 right-4 animate-spin text-red-500/50">
            ...
            <button onClick={toggleCollapse} className="hidden md:flex absolute top-24 -right-3 z-50 bg-red-600 text-white p-1.5 rounded-full border border-white/20 shadow-xl transform translate-y-[-50%] hover:scale-110 transition-all group">
            ...
            <div className="flex flex-col flex-1 overflow-y-auto py-6 space-y-8 no-scrollbar z-10 overflow-x-hidden">
                ...
                {user.role === 'admin' && (
                    <div onClick={closeMobileMenu}>
                        ...
                        <nav className="space-y-1.5 px-4">
                            {executives.map((exec) => (
                                <Link key={exec.name} to={`/calendar?exec=${exec.name}`} className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} px-3 py-2.5 text-sm font-bold rounded-2xl transition-all duration-300 group ${currentExec === exec.name ? 'bg-red-50 text-red-600 border border-red-100 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-red-600'}`}>
                ...
                <div className={`bg-slate-50/80 backdrop-blur-md rounded-3xl border border-slate-200 transition-all group hover:bg-white hover:shadow-md ${isCollapsed ? 'p-2' : 'p-4'}`}>
                    <div className={`flex items-center ${isCollapsed ? 'justify-center mb-3' : 'justify-between mb-4'}`}>
                        <div className="flex items-center">
                            <div className={`p-1.5 rounded-full ${isOnline ? 'bg-red-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.3)]'} animate-pulse`}></div>
                            {!isCollapsed && <span className={`text-[9px] font-black ml-2 uppercase tracking-widest ${isOnline ? 'text-red-600' : 'text-rose-600'}`}>{isOnline ? 'Online' : 'Offline'}</span>}
                        </div>
                        {!isCollapsed && isSyncing && <RefreshCw className="w-3 h-3 text-red-500 animate-spin" />}
                    </div>

                    <div className={`flex items-center ${isCollapsed ? 'justify-center mb-3' : 'mb-4'}`}>
                        <div className={`p-0.5 rounded-2xl bg-gradient-to-br ${user.role === 'admin' ? 'from-yellow-400 to-orange-500' : 'from-red-400 to-rose-600'} shadow-lg`}>
                            <div className="bg-white p-2 rounded-2xl">
                                {user.role === 'admin' ? <Shield className="h-4 w-4 text-yellow-500" /> : <User className="h-4 w-4 text-red-500" />}
                            </div>
                        </div>
                        {!isCollapsed && (
...
const SidebarLink = ({ to, icon, label, isActive, collapsed }: { to: string, icon: React.ReactNode, label: string, isActive: boolean, collapsed: boolean }) => (
    <Link to={to} className={`flex items-center px-4 py-3.5 text-sm font-bold rounded-2xl transition-all duration-500 group ${isActive ? 'bg-red-600 text-white shadow-lg shadow-red-500/20 ring-1 ring-white/20' : 'text-slate-500 hover:bg-slate-50 hover:text-red-600'} ${collapsed ? 'justify-center' : ''}`}>
        <div className={`h-5 w-5 flex-shrink-0 transition-colors duration-500 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-red-500'}`}>{React.cloneElement(icon as React.ReactElement<any>, { size: 20 })}</div>
